...
public class JChessV2 implements ChessEngine {
    ...
    // Precomputed masks for Passed Pawns
    // [color][squareIndex]
    private static final long[][] PASSED_PAWN_MASK = new long[2][64];
    
    // Precomputed masks for Pawn Chain Support
    // Squares diagonally behind the pawn
    ...
    public double evaluate(Board board, PieceColor activeColor) {
        ...
        long whitePawnsBitboard = board.getBitboard(PieceType.PAWN, PieceColor.WHITE);
        long blackPawnsBitboard = board.getBitboard(PieceType.PAWN, PieceColor.BLACK);
            ...
            if (color == PieceColor.WHITE) {
                ...
                if (type == PieceType.BISHOP) {
                    ...
                    if ((blackPawnsBitboard & PASSED_PAWN_MASK[PieceColor.WHITE.ordinal()][i]) == 0L) {
                        score += 20 + (rank * 10); // Je weiter vorne, desto wertvoller
                    }
                ...
                if (type == PieceType.BISHOP) {
                    ...
                    // Freibauer (Passed Pawn)
                    if ((whitePawnsBitboard & PASSED_PAWN_MASK[PieceColor.BLACK.ordinal()][i]) == 0L) {
                        score -= 20 + ((7 - rank) * 10); // Je weiter vorne, desto wertvoller
                    }
        ...
        // Evaluate pawn structure and Rooks on open files
        long whiteRooks = board.getBitboard(PieceType.ROOK, PieceColor.WHITE);
        long blackRooks = board.getBitboard(PieceType.ROOK, PieceColor.BLACK);
        ...
        for (int file = 0; file < 8; file++) {
            ...
            if ((whiteRooks & fileMask) != 0L) {
                if (wPawns == 0 && bPawns == 0) score += 25; // Open file
                else if (wPawns == 0) score += 12; // Semi-open file
            }
            if ((blackRooks & fileMask) != 0L) {
                if (wPawns == 0 && bPawns == 0) score -= 25; // Open file
                else if (bPawns == 0) score -= 12; // Semi-open file
            }
